...
export async function GET(request: Request) {
  try {
    ...
    try {
      ...
      const prombotRes = await fetch("https://prombot.co.uk:8443/api/travel", {
    ...
    const COUNTRY_KEYS: Record<"uk" | "japan" | "can", string> = {
      uk: "uni",
      japan: "jap",
      can: "can",
    };
    ...
    for (const [key, prombotKey] of Object.entries(COUNTRY_KEYS)) {
      ...
      const xanaxItem = countryData?.stocks?.find(
        (item: any) => item.id === 206,
      ); // 206 هو آي دي الزاناكس
      ...
      if (countryData && xanaxItem) {
        ...
        if (xanaxItem.nextRestock) {
          apiPredictedRestock = Math.floor(
            new Date(xanaxItem.nextRestock).getTime() / 1000,
          );
        }
      } else {
      ...
      let oldState = dbData[key] || {
        ...
        history: [],
        stats: {
          avg_depletion_rate: 0,
          avg_restock_delay: key === "japan" ? 9900 : 7200,
          max_stock: 0,
        },
      };
      ...
      if (
        currentQty > oldState.current_quantity &&
        oldState.current_quantity === 0
      ) {
        last_restock_time = updateTime;
        start_qty = currentQty;
        last_predicted_restock =
          oldState.next_predicted_restock ||
          oldState.last_empty_time + oldState.stats.avg_restock_delay;
        ...
        if (cycleDuration > 0 && cycleDuration < 86400) {
          ...
          history.unshift({
            ...
            restock_delay:
              restockDelay > 3000 && restockDelay < 15000
                ? restockDelay
                : key === "japan"
                  ? 9900
                  : 7200,
